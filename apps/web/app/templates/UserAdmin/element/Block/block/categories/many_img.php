	...
	<div class="table__column">
		...
		<div class="block_content td_parent">
			...
			<div class="multiple-uploader" data-numrow="<?= $rownum ?>" onclick="triggerInput(this, event)">
				<?php if (isset($content['many_images'])) foreach ($content['many_images'] as $img) : ?>
					...
					<?php if (@$img['attaches']['image'][0] != '') : ?>
						...
						<div class="image-container" data-pos="<?= $key ?>">
							...
							<?= $this->Form->hidden("info_contents.{$rownum}.many_images.{$key}._old_image", ['value' => $old, 'class' => 'old_img']); ?>
							<?= $this->Form->hidden("info_contents.{$rownum}.many_images.{$key}.id", ['value' => $img['id'], 'class' => 'old_img_id']); ?>
