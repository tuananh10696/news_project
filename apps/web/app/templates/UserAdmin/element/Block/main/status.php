<?php if (empty($this->request->getQuery('author')) && $this->request->getQuery('author') != 'user') : ?>
...
<?php if ($page_config->slug == 'jobs') : ?>
    <div class="form-group row">
        ...
        <?php $type_work = ['tuyen_dung' => 'Tìm Người', 'tim_viec' =>  'Tìm Việc'] ; ?>
        <div class="col-12 col-md-9 control_value">
            <?= $this->Form->input('job_type', [
                    'type' => 'radio',
                    'value' => @$data['job_type'] != '' ? $data['job_type'] : 0,
                    'options' => !empty($this->request->getQuery('type_work')) ? [$type_work[$this->request->getQuery('type_work')]] : $type_work,
                    'hiddenField' => false
                ]); ?>
        </div>
    </div>
